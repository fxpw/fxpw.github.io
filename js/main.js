...
new Typewriter(".loading-text", {
	strings: ["Here could be your advertisement..."],
	autoStart: true,
	delay: 50,
	cursor: "|",
	deleteSpeed: 10,
...
const projectData = [
	{
		...
		srcURL: "https://github.com/fxpw/vscode-fxpw-theme",
	},
	{
		title: "vscode ai chat",
		subTitle: "vscode extension for chatting whit openai (proxy supported)",
		thumbnail: "../assets/images/vscode.png",
		description:
...
function loadContent(projectData) {
	...
	const ProjectCard = function (data) {
		...
		elem.innerHTML = buildTemplate(
			...
			<a href="{srcURL}" target="_blank" class="card-btn" style="height: 2.35rem; color: #fff; background-color: #68d372; padding: .375rem .75rem; border-radius: .25rem;"><i class="fa-regular fa-file-code"></i>Source Code</a>
		</div>`,
			data
		);
		setTimeout(() => {
	...
	const radius = 400 / (2 * Math.sin(Math.PI / projectData.length+3));
